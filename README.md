...
- Unverified submissions are saved first, then `POST /api/auth/email/request` sends Firebase's standard **Email address verification** (`VERIFY_EMAIL`) message. No password is generated and no account-creation screen is shown. Firebase maintains an identity record internally. Google and optional email OTP remain available.
- In Firebase Authentication → Templates → **Email address verification**, set **Customize action URL** to the frontend entry URL (for production, `https://rsvp.gdglebanon.com/`). Add that frontend domain to Authorized domains. **Email link (passwordless sign-in) is not required**; no new `EMAIL_SIGNIN` emails are sent.
- The backend creates or reuses the Firebase identity and privately obtains the ID token needed to send `VERIFY_EMAIL`. Those server-side tokens are never returned to the requester. `FIREBASE_WEB_API_KEY` must permit the backend's Identity Toolkit requests, and the Admin service account must be able to create Firebase custom tokens.
- The custom action page sends the email's `oobCode` to `POST /api/auth/email/complete`. The backend checks the action type, consumes the Firebase code, confirms the verified identity, rejects reused proofs, and only then issues a session. It never issues a session merely because an email address is already marked verified. The pending submission reference travels inside Firebase's `continueUrl`.
- **Check verification status** reloads the authenticated identity and refreshes its token; browser-local persistence shares successful verification across tabs. If verification happened on a different device, open an unused email link in this browser or verify through Google/OTP here. A public email-status lookup cannot grant access.
- `/api/config` advertises `emailAuth: {method: "verificationLink", verificationRequired: true}`. Protected APIs continue requiring a signed token with `email_verified: true`. Previously issued passwordless links remain supported only for compatibility.
- Standard verification email quotas apply (Firebase currently lists 1,000/day on Spark), with additional local per-IP and per-email rate limits. Enable Firestore TTL on `rsvpEmailVerificationProofs.expiresAt` to clean up consumed-proof hashes. No raw email action codes are stored.
