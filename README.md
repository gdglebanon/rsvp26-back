...
- `AttendeeLogin.jsx` uses Firebase Google popup sign-in or `sendSignInLinkToEmail`. Firebase sends the verification link directly.
- The redirect page is the React entry URL with `?auth=callback`. `RegistrationPortal.jsx` handles Firebase's link parameters, completes `signInWithEmailLink`, removes the consumed callback parameters, and loads `/api/me` with the Firebase ID token.
- If a link opens in another browser, the page asks for the receiving email address. The email is not carried as an authentication credential in the URL.
